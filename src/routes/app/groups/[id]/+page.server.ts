...
export const load: PageServerLoad = async ({ params, locals: { user, supabase } }) => {
    ...
    const { data: group, error } = await supabase
        .from('groups')
        .select('id, name, member_groups(id, name, user_id), charges(name, amount, created_at, member_charges(amount, member_group_id), user_id)')
        .eq('id', groupId)
        .single();
    ...
    const myMemberGroup = members.find(m => m.user_id === user?.id);

    const charges = group.charges;

    group.member_groups = members.map((member) => {
        ...
        for (const charge of charges) {
            ...
            const memberCharge = memberCharges.find(mc => mc.member_group_id === member.id);

            // Find if you participated in this charge
            const yourCharge = memberCharges.find(mc => mc.member_group_id === myMemberGroup.id);
