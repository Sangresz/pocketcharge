...
<div class="space-y-4">
	{#each groups as group}
		<div class="flex items-center space-x-2">
			<Button href={`/app/groups/${group.id}`} size="xl" class="w-full rounded-lg border p-4 transition-colors hover:bg-gray-50">
				<h3 class="font-medium">{group.name}</h3>
			</Button>
			<button onclick={() => { selectedGroup = group; openModal() }} class="cursor-pointer mx-2">
				<Pen class="h-5 w-5" />
			</button>
		</div>
	{/each}
